			...
			<div class="col-sm-8 mt-1">
				...
				elseif (isset($_GET['page'])) {
					...
					if ($page<1) {
						...
						$blogs = ($page*6)-6;
						$query = mysqli_query($con, "SELECT * FROM post ORDER BY id DESC LIMIT $blogs,6");
				}
				...
				//DEFAULT QUERY
					...
					$query = mysqli_query($con, "SELECT * FROM post ORDER BY id DESC LIMIT 0,6");
					$page = 1;
				}
					//$i = 0;
			     ...
			     <div class="card my-3">
			     	<div class="card-body">
						...
						<div style="position:relative;">
							<h5 style="position: absolute; top:20px; left:15px;"> <span class="badge badge-warning p-2"><?php echo $row['category'] ?></span></h5>
							<img class="rounded mt-1" src="uploads/<?php echo htmlentities($row['image']) ?>" width="100%">
						</div>
						<small class="text-muted">By <?php echo htmlentities($row['publisher']) ?> on <?php echo htmlentities($row['date']) ?></small>
						<small style="float: right">Comments: 
								<?php
								$id = $row['id'];
						...
						<a href="fullpost.php?id=<?php echo $row['id'] ?>" class="pull-right">
